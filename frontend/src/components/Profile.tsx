...
const ProfileWeb = (props: any) => {
  ...
  useEffect(() => {
    if ((!userImages.profile && !userImages.cover) || !user?.data?._id) return;

    const updateUser = async () => {
      const updatedFields: { profile?: string; cover?: string } = {};

      if (userImages.profile) updatedFields.profile = userImages.profile;
      if (userImages.cover) updatedFields.cover = userImages.cover;
    ...
    updateUser();
  }, [userImages.profile, userImages.cover]);
        ...
        <div className="w-full h-full px-10 lg:px-20 xl:px-60">
          <div className=" top-0 w-full h-64 rounded-2xl overflow-hidden border-box relative group">
            {isLoading?.cover ? (
              ...
              <div className="absolute px-5 py-2 bottom-4 right-4 flex-row text-[var(--background)] text-sm gap-2 rounded-lg bg-black/40 flex justify-center items-center opacity-0 group-hover:opacity-100 transition-all duration-200 cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleFileUpload(e, "cover", setUserImages, setIsLoading)
                  }
                />
              </div>
            )}
            <img
              ...
              src={
                user?.data?.cover ||
                "https://static1.howtogeekimages.com/wordpress/wp-content/uploads/2024/09/macos-sequoia-official-wallpaper.jpg"
              }
            />
          </div>
